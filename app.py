...
# 确保 LengthMeters 为整数
data["LengthMeters"] = data["LengthMeters"].fillna(0).astype(int)
...
# Length (Meters)
length_opts = ["All"] + sorted(data["LengthMeters"].unique().tolist())
...
if selected_length != "All":
    # selected_length is int
    df = df[df["LengthMeters"] == selected_length]
